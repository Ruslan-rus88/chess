...
class MemoryGame {
  ...
  setupEventListeners() {
    ...
    document.getElementById('memory-game-option').addEventListener('click', () => {
    ...
    document.getElementById('memory-game-modal').addEventListener('click', (e) => {
    ...
    document.getElementById('memory-change-level-btn').addEventListener('click', () => {
      this.showLevelSelection();
    });
  }
  ...
  setScreen(name) {
    document.getElementById('memory-level-selection').style.display  = name === 'level'   ? 'block' : 'none';
    document.getElementById('memory-loading').style.display           = name === 'loading' ? 'flex'  : 'none';
    document.getElementById('memory-game-area').style.display         = name === 'game'    ? 'block' : 'none';
    document.getElementById('memory-win-screen').style.display        = name === 'win'     ? 'flex'  : 'none';
  }
  ...
  startGame() {
    ...
    const selectedChars = [...shuffled.slice(0, localCount), this.randomCharacter];
    ...
    document.getElementById('memory-matched').textContent   = `0 / ${this.totalPairs}`;
  ...
  onWin() {
    ...
    setTimeout(() => {
      document.getElementById('memory-win-attempts').textContent = this.attempts;
